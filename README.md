...
The Juno FDxICSP 1 is a self-programmable programmer that is created with the best hardware and software in a tiny package. 1, a bootloader can be anything, and it's implemented with a self-programmable mechanism to upload new code to the application section of the MCU; FDxboot v1.61 is the name of the firmware for the bootloader, and it has achieved a maximum speed of 36.1kB/s upload (write) and 51.1kB/s read; 2, A programmer is the hardware responsible to write (program) to another MCU, so that the target MCU can run with the newly uploaded machine code. FDxICSP v1.81 is the name for the firmware, and it has achieved a maximum speed of 28.3kB/s upload (write) and 32.2kB/s read;
